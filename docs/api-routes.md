...
| `/api/sessions/[id]/checkout-preview` | GET | Xem trước hoá đơn checkout (thành tiền, khuyến mãi, phí gửi xe) |
...
| `/api/reports/shifts` | GET | Danh sách ca kèm doanh thu/giao dịch (cho màn Báo cáo) |
...
| `/api/shifts/[id]/transactions` | GET | Giao dịch (invoice + payment) phát sinh trong ca, kèm đối soát |
| `/api/invoices/[id]` | GET | Chi tiết hoá đơn (items, payments, customer, session, shift, staff) |
| `/api/invoices/[id]/void` | POST | Huỷ hoá đơn đã thanh toán: CANCELLED + Payment âm (refund) + hoàn kho VOID + ghi INVOICE_VOID |
...
- `POST /api/sessions`: check-in 1 khách; hội viên phải có membership còn hạn; vãng lai snapshot giá giờ + tiers; hỗ trợ `playerCount` và `groups` (nhiều nhóm giá trong 1 session — mỗi nhóm tạo `SessionPricingGroup`).
- `POST /api/sessions/[id]/checkout`: checkout tạo `Invoice`, `InvoiceItem(PLAY_TIME)`, `Payment`, cập nhật session/customer; hỗ trợ checkout theo `pricingGroupId`, khuyến mãi (snapshot), phí gửi xe (`SURCHARGE`).
- `POST /api/sessions/[id]/sell`: bán kèm sản phẩm/dịch vụ giữa phiên qua `sellItems()` — tạo invoice, trừ kho, gắn ca.
- `POST /api/invoices/[id]/void`: huỷ hoá đơn qua `voidInvoice()` — đánh dấu CANCELLED, tạo Payment âm (refund), hoàn kho `StockMovement(VOID)`, trừ `totalSpent`/`totalHoursPlayed` của khách, ghi `ActivityLog(INVOICE_VOID)`. Áp dụng cho cả ca đã đóng (điều chỉnh bản ghi admin).
...
- `GET /api/reports/shifts/[id]` và `GET /api/shifts/[id]/transactions`: chi tiết ca — tổng hợp payment theo phương thức, tổng doanh thu, số giao dịch, danh sách hoá đơn/đơn hàng từ `Invoice.shiftId` kèm `items`, `payments`, `customer`, `session`, `staff`. `STAFF` chỉ xem ca mình mở hoặc tham gia; `ADMIN` xem mọi ca.
