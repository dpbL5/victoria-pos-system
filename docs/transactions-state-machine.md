...
Mô hình hoá vòng đời của **giao dịch** trong hệ thống POS — từ lúc phiên chơi được tạo, phát sinh hoá đơn/thanh toán, đến đối soát ca và điều chỉnh (void). **Giao dịch** là bản ghi `Payment` với 2 loại (`Payment.kind`):

- `Payment(kind=OPERATIONAL)` — thanh toán hoá đơn vận hành (checkout, bán kèm)
- `Payment(kind=MEMBERSHIP)` — phí hội viên (trước đây là bảng `MembershipPayment` riêng, đã gộp qua single-table inheritance)

Đây cũng chính là nguồn dữ liệu của màn Giao dịch (`/transactions` → `getShiftTransactions`) và báo cáo ca (`/api/reports/shifts/[id]`).
...
4. **MEMBER không phải tiền mặt**: thanh toán qua hội viên không thu tiền mặt tại quầy → vào `totalRevenue` nhưng **không** vào `expectedCash` (xem `calculateExpectedCash` trong `src/lib/shifts/helpers.ts`).
...
│ (phiên chơi)│            │  (hoá đơn)  │             │  kind=OPERATIONAL     │
└─────────────┘            └─────────────┘             └──────────────────────┘
        ▲                       ▲              kind=MEMBERSHIP (phí hội viên)
        │                       │             ┌──────────────────────────┐
...
Không có trường status — **immutable, append-only**. Mỗi `Payment` gắn `invoiceId` + `shiftId` + `staffId` (người thao tác, để truy vết trách nhiệm), và `kind` (`OPERATIONAL`/`MEMBERSHIP`).

| Nguồn sinh | kind | Phương thức | Số tiền | Ghi chú |
|---|---|---|---|---|
| `checkout` | `OPERATIONAL` | `CASH`/`TRANSFER`/`CARD`/`MEMBER` | dương | 1 invoice = 1 payment; hội viên checkout mặc định `MEMBER` (không thu tiền mặt tại quầy) |
| `registerMember` / `renewMembership` | `MEMBERSHIP` | tuỳ chọn (thường `CASH`/`TRANSFER`) | dương | invoice `MEM` + `Payment(kind=MEMBERSHIP)` cùng lúc |
| `voidInvoice` | *(không tạo payment mới)* | — | — | chỉ đánh dấu `CANCELLED`; báo cáo doanh thu lọc bỏ payment gốc qua `invoice.status` |

- `MEMBER`: vào `totalRevenue` và `memberRevenue`, **không** vào `cashRevenue`/`expectedCash` (`getShiftRevenueData` trong `src/lib/shifts/helpers.ts`).
...
## 4. Payment(kind=MEMBERSHIP) (giao dịch phí hội viên)

Phí hội viên là `Payment` với `kind = MEMBERSHIP` (gộp từ bảng `MembershipPayment` cũ qua single-table inheritance) — đính kèm `membershipId`/`planId`/`customerId`.

| Sự kiện | Từ → Đến | Guard | Side effects | Nguồn |
|---|---|---|---|---|
| `đăng ký hội viên` | (mới) | `SHIFT_REQUIRED`; `PLAN_NOT_FOUND` | 1 transaction: customer + membership `ACTIVE` + invoice `MEM` `PAID` + `Payment(kind=MEMBERSHIP)`; `ActivityLog(MEMBERSHIP_REGISTER)` | `use-cases/registerMember.ts` |
...
- `Payment(kind=MEMBERSHIP)` luôn có `plan.name` qua relation — trong màn Giao dịch (`getShiftTransactions`), dòng này map thành `type: 'membership'`, gắn `invoiceNo` của invoice `MEM` (có thể click).
...
| Sự kiện | Use-case | Invoice | Payment | StockMovement | ActivityLog |
|---|---|---|---|---|---|
| Check-in | `checkIn` | — | — | — | `SESSION_CHECK_IN` |
| Bán hàng trong phiên | `sellItems` | `DRAFT` | — | `SALE` | `SESSION_SELL` |
| Checkout | `checkOut` | `DRAFT → PAID` | **+1** (OPERATIONAL) | `SALE` | `SESSION_CHECK_OUT` |
| Đăng ký hội viên | `registerMember` | `MEM PAID` | +1 (MEMBERSHIP) | — | `MEMBERSHIP_REGISTER` |
| Gia hạn hội viên | `renewMembership` | `MEM PAID` | +1 (MEMBERSHIP) | — | `MEMBERSHIP_RENEW` |
...
1. `Payment` luôn gắn đúng 1 `invoice`. Khi huỷ hoá đơn, payment gốc giữ nguyên (không tạo refund) — báo cáo doanh thu lọc bỏ qua `invoice.status != 'CANCELLED'`.
2. Không sinh `Payment` từ invoice `DRAFT` — giao dịch chỉ ra đời khi invoice `PAID`.
3. Mọi giao dịch gắn `shiftId` của ca đang mở; ca `CLOSED` không nhận giao dịch mới (trừ correction).
4. `Payment` method `MEMBER` (không thu tiền mặt tại quầy) → vào `totalRevenue` nhưng không vào `expectedCash` khi đóng ca.
5. Session `COMPLETED`/`CANCELLED` là terminal — không checkout lại, không bán thêm. `thu trước` (partial checkout) giữ session `ACTIVE` tới khi thu hết người.
...
| Thành phần | Vị trí |
|---|---|
| Enum trạng thái | `prisma/schema.prisma` (`SessionStatus`, `InvoiceStatus`, `ShiftStatus`, `PaymentKind`) |
| Use-cases (máy trạng thái thực thi) | `src/lib/<domain>/use-cases/` — `checkIn`, `sellItems`, `checkOut`, `registerMember`, `renewMembership`, `voidInvoice`, `openOrJoinShift`, `closeShift` (mỗi file có `mapXxxError` chuyển guard → HTTP) |
| Giao dịch đọc (read model) | `src/lib/shifts/helpers.ts` — `getShiftTransactions` (Payment theo `kind`), `getShiftRevenueData`, `calculateExpectedCash` |
...
| Kiểm toán | `src/lib/audit/helpers.ts` (`logActivity`) |

## Ngoài phạm vi

- Không đổi code — tài liệu thuần mô hình hoá hành vi hiện có.
- Không thêm trường status cho `Payment` (giữ immutable append-only, `kind` chỉ phân loại giao dịch).
